...
function asyncMedia(videoId, audioId) {
    ...
    Promise.all([
        ...
        var videoURL = 'http://res.cloudinary.com/wulihh/video/upload/e_volume:-100/e_volume:400,l_video:'
            + audio_public_id + '/' + video_public_id + '.mp4'
        var thumbName = video_public_id.replace('/', '_') + '.jpg';
        var thumbURL = 'http://res.cloudinary.com/wulihh/video/upload/' + video_public_id + '.jpg'

        console.log('同步视频到七牛')
        console.log('视频地址：' + videoURL)
        console.log('视频名字：' + videoName)
        console.log('头像地址：' + thumbURL)
        console.log('头像名字：' + thumbName)
        robot
...
exports.save = function *(next) {
    ...
    if (!creation || creation.finish < 100) {
        var creationData = {
            author: user._id,
            title: xss(title),
            audio: audioId,
